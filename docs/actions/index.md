...
```typescript
// sites/mysite/.deploy/actions/api-key-action.ts
...
- **Site-specific actions**: Located in the site's `.deploy/actions` directory
  - Example: `sites/mysite/.deploy/actions/nightly-build.ts`
  - Legacy location (still supported): `sites/mysite/actions/nightly-build.ts`
- **Root-level actions**: Located in the root `.deploy/actions` directory
  - Example: `.deploy/actions/system-backup.ts`

The system will first look for actions in the `.deploy/actions` directory, and then fall back to the legacy `actions` directory if needed.
...
```typescript
// sites/mysite/.deploy/actions/build-action.ts
import { defineScheduledAction } from "@keithk/deploy-actions";
...
```typescript
// sites/mysite/.deploy/actions/deploy-action.ts
import { defineScheduledAction } from "@keithk/deploy-actions";
...
```typescript
// sites/mysite/.deploy/actions/build-and-deploy.ts
import { defineScheduledAction } from "@keithk/deploy-actions";
