...
export function renderDeployScreen(siteName: string, statusText: string, pollUrl?: string): string {
  ...
  return [
    ...
    pollUrl
      ? '  <div class="hint" id="hint">this page will refresh automatically when the site is ready</div>'
      : '',
    '</div>',
    '',
...
function buildPollScript(pollUrl: string): string {
  ...
  return [
    ...
    '      if (statusEl) statusEl.textContent = "still not ready";',
    '      if (hintEl) hintEl.textContent = "this is taking longer than expected - refresh to check again";',
    '      return;',
    '    }',
    ...
    '          if (hintEl) hintEl.textContent = "check the deploy logs in the admin dashboard";',
    '        } else {',
