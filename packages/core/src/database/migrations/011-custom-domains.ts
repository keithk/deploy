// ABOUTME: Migration adding custom_domain column to sites for custom domain support.
// ABOUTME: Enables DB-backed sites to have a custom domain (e.g., myapp.com).

import type { Database } from "../database";

export const name = "011-custom-domains";
...
export function down(db: Database): void {
  ...
  db.run(`
    INSERT INTO sites_old (id, name, git_url, branch, type, visibility, status, container_id, port, env_vars, persistent_storage, autodeploy, created_at, last_deployed_at, sleep_enabled, sleep_after_minutes, last_request_at, compose_yaml, primary_service, primary_port)
    SELECT id, name, git_url, branch, type, visibility, status, container_id, port, env_vars, persistent_storage, autodeploy, created_at, last_deployed_at, sleep_enabled, sleep_after_minutes, last_request_at, compose_yaml, primary_service, primary_port
    FROM sites
  `);
