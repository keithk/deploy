...
export async function migrateToMise(siteName: string, sitesDir: string = './sites'): Promise<void> {
  ...
  if (!existsSync(sitePath)) {
    console.error(`❌ Site '${siteName}' not found at ${sitePath}`);
    process.exit(1);
  }
  ...
  if (existsSync(miseConfigPath)) {
    console.log(`⚠️  Site '${siteName}' already has a .mise.toml file`);
  ...
  console.log(`🔧 Generating mise configuration for site: ${siteName}`);
  ...
  console.log(`✅ Created .mise.toml for ${siteName}`);
...
export async function listMiseSites(sitesDir: string = './sites'): Promise<void> {
  ...
  try {
    ...
    console.log('\n📋 Sites status:');
    sites.forEach(site => {
      const status = site.hasMise ? '✅ Mise' : '📦 Traditional';
    ...
    console.log(`\n📊 Summary: ${miseCount}/${totalCount} sites using mise`);
    
    if (miseCount < totalCount) {
      console.log('\n🔧 To migrate remaining sites:');
